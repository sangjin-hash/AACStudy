...
class LoginFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.loginState.observe(viewLifecycleOwner, Observer<Int>{ tag ->
            when(tag){
                1 -> Toast.makeText(activity, "아이디 & 비밀번호가 비어있습니다",Toast.LENGTH_SHORT).show()
                2 -> Toast.makeText(activity, "아이디가 비어있습니다",Toast.LENGTH_SHORT).show()
                3 -> Toast.makeText(activity, "비밀번호가 비어있습니다",Toast.LENGTH_SHORT).show()
            }
        })
